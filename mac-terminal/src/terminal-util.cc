...
terminal_util_show_help (const char *topic)
{
  ...
  if (topic) {
    uri = g_strdup_printf ("help:gnomac-terminal/%s", topic);
  } else {
    uri = g_strdup ("help:gnomac-terminal");
  }
...
terminal_util_show_about (void)
{
  ...
  dialog = (GtkWindow*)g_object_new (GTK_TYPE_ABOUT_DIALOG,
                         ...
                         "logo-icon-name", GNOMAC_TERMINAL_ICON_NAME,
                         nullptr);
...
{
  const gchar *license[] = {
    N_("GnomacTerminal is free software: you can redistribute it and/or modify "
       "it under the terms of the GNU General Public License as published by "
       "the Free Software Foundation, either version 3 of the License, or "
       "(at your option) any later version."),
    N_("GnomacTerminal is distributed in the hope that it will be useful, "
       "but WITHOUT ANY WARRANTY; without even the implied warranty of "
       "MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the "
       "GNU General Public License for more details."),
    N_("You should have received a copy of the GNU General Public License "
       "along with GnomacTerminal.  If not, see <http://www.gnu.org/licenses/>.")
  };
...
terminal_util_timestamp_info (const char *str)
{
   ...
   * right here and check the human-readable time in gnomac-terminal.
 ...
 *
 * Checks if gnomac-terminal should attempt to handle the given URI,
 ...
 *
 * Returns: The possibly rewritten URI if gnomac-terminal should attempt
...
#define TERMINAL_CACHE_DIR                 "gnomac-terminal"
